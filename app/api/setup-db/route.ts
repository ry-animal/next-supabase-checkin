...
export async function GET() {
  try {
    ...
    const { data, error: tableCheckError } = await supabase.from('users').select('id').limit(1);
    ...
    if (tableCheckError) {
      if (tableCheckError.code === '42P01') {
        ...
        return NextResponse.json(
          {
            message:
              'The users table does not exist. Please run the following SQL in your Supabase SQL Editor:',
            sql: `
